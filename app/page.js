...
export default function HomePage() {
  const [healthStatus, setHealthStatus] = useState({ loading: true, status: null });
  const [dbStatus, setDbStatus] = useState({ loading: true, status: null });
  ...
  useEffect(() => {
    async function checkHealth() {
      try {
        ...
        setHealthStatus({ 
          loading: false, 
          status: { status: 'error', message: 'Failed to check health status' } 
        });
      }
    }
    ...
    async function checkDbHealth() {
      try {
        ...
        setDbStatus({ 
          loading: false, 
          status: { status: 'error', message: 'Failed to check database status' } 
        });
      }
    }
